...
function simpleBarChart(input)
{
   ...
   {
      var id = d['id'];
      if (id.substr(0, 3) == "en:" || id.substr(0, 3) == "fr:" || id.substr(0, 3) == "de:" || id.substr(0, 3) == "es:" || id.substr(0, 3) == "nl:")
      {
         id = id.substr(3);
      }
   ...
   {
      return parseFloat(d['products']);
   };
   var barLink = function(d)
   {
      return d['url'];
   };
   // Scales  
   ...
   var chart = d3.select('#chart').append("svg").attr('width', maxBarWidth + barLabelWidth + valueLabelWidth).attr('height', gridLabelHeight + gridChartOffset + data.length * barHeight);
   // Grid line labels  
   ...
   labelsContainer.selectAll('text').data(data).enter().append('text').attr('y', yText).attr('stroke', 'none').attr('fill', 'black').attr("dy", ".35em")
   ...
   barsContainer.selectAll("rect").data(data).enter().append("rect").attr('y', y).attr('height', yScale.rangeBand()).attr('width', function(d)
   {
      return x(barValue(d));
   }).attr('stroke', 'Gray').attr('fill', '#ccccb3');
   // Bar value labels  
   barsContainer.selectAll("text").data(data).enter().append("text").attr("x", function(d)
   {
      return x(barValue(d));
